...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: `${t("items.reportAging")} - Sanaeva Store` };
}

export default async function ReportAgingPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = (await params) as { locale: Locale };
  const t = createBackofficeTranslator(locale, "sidebar");
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>GET /api/reports/aging</CardDescription>
        </CardHeader>
